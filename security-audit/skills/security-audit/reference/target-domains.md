...
Domain-specific attack classes, tools, and entry points.

## Smart Contracts (Solidity, Vyper, Rust/Solana, Move)

### Attack classes
- **Reentrancy** — external calls before state updates (`call.value()` before balance decrement)
- **Integer overflow/underflow** — unchecked math on token amounts (pre-Solidity 0.8)
- **Access control** — missing `onlyOwner`, unprotected `selfdestruct`, uninitialized proxy owners
- **Flash loan attacks** — price oracle manipulation via single-transaction liquidity shifts
- **Front-running / MEV** — transactions visible in mempool before inclusion
- **Delegatecall injection** — storage layout mismatch in proxy patterns
- **Logic bugs** — incorrect reward calculations, rounding errors favoring attackers
- **Signature replay** — missing nonce or chain ID in signed messages
- **Unchecked return values** — low-level calls that fail silently

### Tools
- Slither (static analysis), Mythril (symbolic execution), Echidna (fuzzing), Foundry (testing)
- `solc --asm` for opcode-level analysis
...
- **Device drivers** — `ioctl` handlers, DMA operations, race conditions in device state
- **Filesystem handlers** — path traversal in VFS operations, symlink races
- **Netfilter/eBPF** — verifier bypasses, JIT spray, bounds check evasion
- **Memory management** — slab overflow, use-after-free in SLUB allocator, page table manipulation
...
### Attack classes
- **Default credentials** — hardcoded admin passwords, API keys in firmware
- **Command injection** — web management interfaces passing input to shell
- **Firmware extraction** — binwalk to extract filesystem, find hardcoded secrets
- **UART/JTAG access** — physical debug interfaces left enabled
- **Buffer overflows** — C/C++ web servers and CGI handlers without bounds checking
- **Unencrypted protocols** — Telnet, HTTP management, MQTT without TLS
- **UPnP/SSDP abuse** — device discovery and port mapping exploitation
- **DNS rebinding** — accessing internal device management from external networks
...
- **Prototype pollution** — deep merge operations on user objects
- **ReDoS** — catastrophic regex backtracking on user input
- **Path traversal** — `path.join` with user input (doesn't sanitize `../`)
- **Dependency confusion** — private package names squatted on public registries
- **eval/Function constructor** — user input reaching code execution
- **Deserialization** — `node-serialize`, `js-yaml` unsafe load

### Python ecosystem
- **Pickle deserialization** — `pickle.loads()` on untrusted data = RCE
- **SSTI** — Jinja2/Mako template injection
- **Import hijacking** — relative imports, `sys.path` manipulation
- **subprocess shell=True** — command injection via shell metacharacters
- **yaml.load without SafeLoader** — arbitrary code execution

### Homebrew / package managers
- **Formula/cask injection** — malicious formulae, typosquatting
- **Build script execution** — post-install scripts with elevated privileges
- **Dependency chain attacks** — compromising transitive dependencies

### Rust (yes, even Rust)
- **unsafe blocks** — manual memory management, FFI boundaries
- **Integer overflow in release mode** — wrapping behavior (not panicking)
- **Logic bugs** — Rust prevents memory bugs, not logic bugs
- **FFI boundaries** — calling C code from Rust loses all safety guarantees
- **Concurrency bugs** — data races are prevented, but deadlocks and logic races aren't

### Go
- **Integer overflow** — no automatic checking
- **Race conditions** — goroutine data races (use `-race` flag)
- **HTTP request smuggling** — Go's HTTP server quirks
- **Template injection** — `text/template` vs `html/template` confusion
- **Slice header aliasing** — shared underlying arrays after append

## CI/CD Pipelines

### GitHub Actions
- **Unpinned actions** — `uses: action@main` instead of `@sha256:...`
- **Expression injection** — `${{ github.event.pull_request.title }}` in `run:` steps
- **pull_request_target** — runs with write permissions on PRs from forks
- **Artifact poisoning** — malicious artifacts from fork PRs consumed by trusted workflows
- **Secret exfiltration** — secrets available to steps that shouldn't have them
- **AI action injection** — Claude Code Action, Codex, Gemini CLI with env var intermediaries

### General CI/CD
- **Pipeline poisoning** — modifying CI config in a branch to exfiltrate secrets
- **Dependency confusion in builds** — internal package names squatted during CI
- **Build cache poisoning** — injecting malicious cached artifacts

## Cloud Infrastructure

- **Overly permissive IAM** — `*:*` policies, roles assumable by any service
- **Public S3/GCS/Blob** — storage buckets readable/writable without auth
- **IMDS exploitation** — instance metadata service (169.254.169.254) accessible from application code
- **Serverless injection** — event data reaching `eval` or shell execution in Lambda/Cloud Functions
- **Container escape** — privileged containers, mounted Docker socket, host PID namespace
- **Terraform state** — secrets in state files, state stored without encryption

## Bug Bounty Prioritization

| Tier | Severity | Typical payout | Examples |
|------|----------|---------------|----------|
| 1 | Critical | $5k-$100k+ | RCE, auth bypass on critical systems, full account takeover |
| 2 | High | $1k-$20k | SQL injection with data access, stored XSS on sensitive pages, SSRF to internal services |
| 3 | Medium | $500-$5k | IDOR, CSRF on important actions, information disclosure of secrets |
| 4 | Low | $100-$1k | Reflected XSS, missing headers, open redirect |

Focus effort on Tier 1-2. Read the program's scope and exclusions carefully before testing.
