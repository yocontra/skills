# Report Template

Start with the structured verdict block (see Interface in SKILL.md), then this narrative structure. Print directly to user.

```markdown
# Vulnerability Report: ${TARGET_NAME}
**Date:** ${DATE}
**Mode:** White-box
**Scope:** ${FILES_SCANNED} files across ${DIRECTORIES} directories
**Methodology:** Adversarial agent audit (${N_FINDERS} finders, disprovers, refereed)

## Executive Summary
- **Critical:** N findings
- **High:** N findings
- **Medium:** N findings
- **Low:** N findings
- Total files scanned: N
- Findings submitted by finders: N
- Findings killed by disprovers: N
- Findings approved by referee: N

## Confirmed Findings

### [VULN-001] ${TITLE}
**Severity:** Critical/High/Medium/Low
**Bug class:** ${BUG_CLASS}
**Location:** ${FILE}:${LINE}
**Referee verdict:** ${VERDICT}
**Confidence signals:** ${LIST_OF_INDEPENDENT_SIGNALS}

**Description:**
${WHAT_THE_BUG_IS}

**Attack Vector:**
${HOW_ATTACKER_REACHES_IT}

**Disprover's best objection:**
${STRONGEST_COUNTERARGUMENT_AND_WHY_IT_FAILED}

**Impact:**
${WHAT_ATTACKER_ACHIEVES}

**Chain potential:**
${HOW_THIS_COMBINES_WITH_OTHER_FINDINGS}

**Remediation:**
${SUGGESTED_FIX}

---

## Disputed Findings (CONDITIONAL verdict)
(same format, with conditions noted)

## Variant Analysis
For each confirmed finding, list other locations in the codebase with the same pattern:
| Finding | Pattern | Other locations | Confirmed? |
|---------|---------|-----------------|------------|

## Killed Findings (for transparency)
| # | Title | Finder | Disprover | Kill reason |
|---|-------|--------|-----------|-------------|

## Coverage Gaps
(zones where finders returned nothing or agents failed)
```
